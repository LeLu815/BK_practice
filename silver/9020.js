 ...
 * 
 * 합과 차는 한끝 차이이다.
 * 두 수의 합이 n 이 나온다는 조건은 n 과 한 수 의 차가 다른 한수여도 된다는 것을 의미한다.
 * 두 수의 차는 후반부로 갈수록 작아진다.
 * 위의 두 개념을 적용하면 연산과정을 효율적으로 줄일 수 있다.
 */
...
const combination = (list, num) => {
    ...
    for(let i=0; list[i]<num/2+1; i++) {
        ...
        //     return ;
        // });
        const index = list.indexOf(num - list[i]);
        if (index !== -1) {
            answer.push([list[i], list[index]]);
        }
    }
...
for (let i=1; i<=input.at(0); i++) {
    const combiList = combination(mainList, input[i]);
    console.log(combiList.pop().join(' '));
    // console.log(combiList.pop());
